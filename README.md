...
38. 🔬 **借券賣出大增/回補 事件研究**(`tw_sbl_surge_study.py`,2026-08-01)— 回答「借券賣出大增後股價怎麼走?大量後陸續回補又怎麼走?」。全市場 4 位數普通股、近一年(2025-07~2026-07,250 交易日、1937 檔),還原收盤計酬、超額=減同期加權。事件:**A 大增**=10日餘額增≥50% 且 +≥1000張 且基期≥500張(979 次);**B 回補**=60日峰值≥3000張、首次跌破峰值 70%(1296 次);同檔冷卻 40 日。**結論**:①大增前 20 日平均 +6.4% → 空單多建在強勢股上。②**「被大量借券空=會跌」不成立**:大增後 1-2 週中性偏弱(H10 超額中位 −3.4% ≈ 對照 −3.2%),但 **H60 絕對 +20.3%/勝率 67%,遠勝對照 +7.7%/47%**,超額均 −7.3% vs 對照 −12.6%(+5.3pp)——動能延續+軋空燃料,分布右偏(中位 +8.2%,靠大贏家)。③**回補後 5-10 日是全表唯一超額轉正視窗**(H5 +0.07%/H10 +0.15% vs 對照 −0.97%/−1.98%,勝率 54%),空方回補買盤+風險解除;H60 絕對 +16.4%/勝率 61%。④⚠ regime:本年平均個股超額 −12.6%/60日(權值撐盤),超額須「vs 對照」讀;樣本僅一年含兩次崩盤;回補=還券動機不唯一(召回/自主)。資料:TaiwanDailyShortSaleBalances 逐日全市場(重用 margin-scan `_sbl_day` 快取 `cache/sbl_day/`,已補 250 天)+ year_prices 還原收盤。結果 `cache/sbl_surge_study.json`。CLI `--backfill`/`--json-out`。
