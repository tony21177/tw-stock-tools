...
40. 🧪 **策略回測總覽 `/backtests`**(2026-08-01,用戶要求「所有回測同一頁,不另開頁面」)— 六個回測頁(族群策略/第二波/轉機接力/借券/盤中模擬/主力雷達)整合為單頁:`backtests_page()` 直接呼叫各原 view 函式、抽出 body(去 script/nav/h1)與 style 拼接,頁頂 TOC 錨點導覽。舊網址全部 **301 → `/backtests#錨點`**(書籤/舊連結兼容);首頁 g-backtest 六 tab 併為單一「🧪 策略回測總覽」tab;站內所有舊回測連結(premarket renderer 的第二波說明連結、各頁第二排 nav)已全改錨點。同日 **`/chip-compare`(兩波比對)下架**(用戶不再使用):route 移除、首頁快查按鈕/tab、chip-price 第二排連結全清(模組 `tw_chip_episode_compare.py` 檔案保留未刪)。
