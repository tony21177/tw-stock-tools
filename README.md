...
7. **🧭 分點行為分類** (2026-07-17 加) — 依券商名稱把分點分成 **🌏 外資 / 🏠 散戶指標 (永豐金、國泰敦南) / 🏦 內資** 三組，各列買賣超 Top 5 + 合計淨額，並對每個分點貼行為標籤：**低接/追高**（買均價在當日高低區間的位階 ≤35% / ≥65%）、**當沖**（買賣量相抵 ≥70%）、**隔日沖**（昨淨買 ≥ 門檻且今淨賣 ≥ max(門檻, 昨買 40%)，需前一交易日 `bsr_cache/*_prices.json`）、**純倒貨**（買 < 賣的 15%）、**殺低出**。門檻 = max(100 張, 當日總量 0.2%)，**先驗未回測**。另輸出 ⚡ 隔日沖名單 + 🔁 當沖分點（顯示上限 10 個）。⚠ 外資分點含客戶委託、非全為外資自營；散戶指標為經驗 proxy；判讀時不可只挑符合敘事的分點（外資有人低接時要同時看有沒有外資在倒）
