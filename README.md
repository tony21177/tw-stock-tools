...
21. **TSM ADR vs 2330 折溢價** — TSM (台積電 ADR) 與台股 2330 的折溢價歷史，可選 1-10 年區間。換股比例 1:5，理論價 = TSM(USD)×USD/TWD÷5，折溢價 = (理論價/2330實際價−1)。資料 Yahoo (TSM/2330.TW/TWD=X 日收盤)，含摘要卡（當前/均值/區間高低/百分位）+ 折溢價折線圖（含均值線）+ 近 20 日明細表。⚠ 時間差 (TSM 收盤晚 2330 約 14.5h，溢價為隔日開盤跳空前瞻指標) + 除權息假性折溢價 caveat（2026-06-04 加）→ `tw_adr_premium.py` · `/adr-premium?years=N`
