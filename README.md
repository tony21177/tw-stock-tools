...
20. **前十大股東 + 集保大戶分布** — 輸入股號查 (a) MOPS 年報「主要股東名單」前十大股東（姓名 + 持有股數 + 持股比例 + 停止過戶日，自動下載 F04 年報本文 PDF；用 extract_tables + text 雙解析容錯各家不同表格 layout，主要股東名單表優先、關係人表 fallback、序號/簡稱/負責人雜訊清理、<5 筆視為解析失敗、負快取避免重掃；JSON 快取秒回）；(b) 集保 TDCC 每週股權分散表，4 群組摘要（散戶/中實戶/大戶/千張大戶）+ 籌碼集中度趨勢線（千張大戶 vs 散戶）+ 級距長條圖 + 15 級距表（2026-05-28 加）→ `fetch_major_shareholders()` in `mops_pdf.py` · `fetch_holding_distribution()` in `finmind_client.py` · `/shareholders?code=X`
