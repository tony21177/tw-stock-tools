...
19. **分點 N 日時段 pattern** — 給定 (股號, 分點)，自動算過去 N 日該分點在早/中/尾盤的買賣分布 + 配對 OHLC 走勢 + 6 種行為標籤（尾盤低接 / 早盤追擊 / etc）+ 可展開的專有名詞詳細解讀（2026-05-20 加）→ `broker_timing_pattern()` in `tw_chip_price.py` · `/chip-price?code=X&broker=Y`
    - **🤖 AI 行為敘事按鈕** (2026-07-17 加) — `/chip-price` 報告頁一鍵觸發：把「近 10 日分點連續買賣序列」+ 判讀紀律餵給**本機 headless Claude CLI** (`claude -p`)，產生範例文風格的敘事分析（外資動向 → 內資逐分點點名 → 散戶 → 結論 + 明日觀察重點），約 1-2 分鐘，完成後自動顯示在頁面上。背景 thread + 檔案狀態機（`cache/chip_narrative/{code}_{date}.json` + `.status.json`，跨 process 安全、7 分鐘 stale 自動可重試）；同 (code, date) 有快取直接顯示（**每次產生 = 一次 Claude 用量**），可按「重新產生」強制重跑。判讀紀律 prompt 內嵌於 `concept_momentum/chip_narrative.py`（源自 chip-price skill 的行為序列判讀紀律：隔日沖要完整 cycle、真累積 vs 沖來沖去、不可挑分點）。CLI 測試：`python3 chip_narrative.py 2313 20260716 [--prompt-only]`。API：`POST/GET /api/chip-narrative?code=&date=`
