...
為避免低流動性個股稀釋族群訊號（例如 5274 信驊每日只成交 ~300 張、單價 1.6 萬，雖然概念正確但不能代表族群動能）：

1. **過濾**：20d 平均張數 < 500 張的成分股不納入動能計算（`MIN_AVG_LOTS = 500`）
2. **加權**：族群指數從等權重改為「成交額加權」（成交金額 = 收盤 × 成交股數，越高的股票權重越大）

例外：rerating_detector 使用獨立的等權重指數（`concept_index_equal`），避免相關性分析受過濾或加權變動影響漂移。
...
匹配規則（避免子字串誤觸發）：
- 中文關鍵字、長英文（>5 字）：直接子字串匹配
- 短英文（≤5 字、純 ASCII）：要求前後不能是字母或數字（word boundary）
  例：「ESS」不會匹配「PharmaEssentia」、「ASIC」不會匹配「classic」、「LEO」不會匹配「Galileo」

**Step 2.5：剝除公司名再匹配**
匹配前先把該股的中文名、代號、`-KY` 後綴前的版本（如「萊德光電-KY」→「萊德光電」）從標題中剝除，避免公司名（含「光電」「電子」等通用字）觸發其他概念。
...
### 已知限制
- 關鍵字匹配為輕量近似法，雖已加入 word-boundary + 公司名剝除避免主要誤判，但仍可能有同義詞遺漏
...
1. **整合 LLM 深度判讀**
   對 rerating_detector 標記出的高動能候選股（5-10 檔），把新聞標題送給 Claude/GPT 判斷主題。
   實作：透過 Anthropic API 或 OpenAI API。每週成本約 USD $0.5-2。
   優點：捕捉同義詞、否定句、語意連動。
   待辦：等 user 提供 API key。

2. **精化關鍵字字典**
   - 排除公司名稱中的歧義詞（「光電」、「綠能」等）
   - 加入「子主題」如 GB300、Blackwell、HBM4 等技術代號
   - 用回測驗證命中率：列出歷史已 rerate 成功的股票，看詞典抓得到嗎

3. **多新聞源整合**
   除 Yahoo TW 外加入 cnyes 鉅亨、Anue 個股新聞、MOPS 重大訊息
   各新聞源權重不同：MOPS（公司自爆轉型）權重高、媒體報導權重中、論壇權重低

4. **語意過濾**
   用 NLP 處理否定（「3665 不再做 EV」）、引述（「分析師認為...」）、預測（「未來可能轉型」）
   差異：「公司宣布轉型」vs「分析師預期會轉型」應有不同權重

5. **時間衰減**
   近期新聞權重較高（過去 7 天 ×1.5、8-30 天 ×1.0），更靈敏抓相位變化

6. **與 rerating_detector 整合 scoring**
   建立統一指標：drift_score = α × news_drift + β × price_rerating
   把兩個 detector 的訊號合成一個排序

7. **回饋學習機制**
   user 確認某些候選為「真轉型」或「假訊號」，下次跑時加權調整字典或 threshold
   類似 active learning
...
- **四象限標記**：🔥 真流入（占比升+法人買）/ ⚠ 出貨疑慮（占比升+法人賣）/ 🧲 低調吸收 / ❄ 退潮。
  門檻 `FLOW_SHARE_PP=0.15`pp、`FLOW_INST_NTD=0.5` 億、且淨流 ≥ 總流量的 `FLOW_NET_GROSS_RATIO=10%`
  （2026-07-15 加入：外資投信大額對沖日的雜訊殘差不給標記 — 07-14 被動元件外資 +66.6 億 vs
  投信 −66.8 億、淨流 +2.77 億曾被誤標 🔥 的真實案例）— **先驗設定，未經回測驗證**。
  注意 🔥 在大跌爆量日也可能出現（外資接刀/回補空單），≠ 看多訊號，需搭配價格判讀。

入口：dashboard「訊號監控」群組 tab（17:00 烤入）＋ `/money-flow` 獨立頁（即時）＋ 動能排行表兩欄 ＋ 每日 TG 摘要（流入/流出 Top5、⚠、連續 ≥5 日）。

同頁附「🌏 外資買賣超」區（2026-07-15 加入）：上市/上櫃外資買賣超金額近 10 日表 ＋
當日外資買超/賣超 Top15 個股（代號/名稱/市場/金額，榜單排除 ETF）。

上市/上櫃兩欄用**官方公布值**（上市: FinMind `TaiwanStockTotalInstitutionalInvestors`；
上櫃: 櫃買中心 open data `tpex.org.tw/www/zh-tw/insti/summary`）— 與新聞數字一致；
缺官方時以「淨股數 × 收盤價」近似回填並加 `~` 標示。個股 Top15 因官方無個股金額、全為近似值。
口徑教訓（2026-07-15 校驗）：近似加總必須含 ETF 等全部證券（外資單日買 ETF +91.6 億，
只算個股會差官方 ~55 億；含 ETF 後殘差僅數億）。
