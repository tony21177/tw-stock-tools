...
def _render_sw(rows: list[dict]) -> str:
    ...
    parts = ['<div class="table-scroll" style="overflow-x:auto;">',
             ...
             '<th title="股票代號">代號</th>'
             '<th title="股票中文名稱">名稱</th>'
             '<th title="分層標記 — 依 2026-07 子群回測 (502 episodes) 分層。距前高 = 今日收盤 ÷ 波段峰值收盤；例：峰值 100 元、今收 85 元 → 85% (<88%=早期)，今收 92 元 → 92% (≥88%=已近前高、追高)：'
             ...
             '&#10;&#10;drop_pct (急跌幅度)：從峰值到低點的跌幅 (e.g., 0.22 = 急跌 22%)。15~25% 是黃金區間，太淺洗盤不夠、太深可能是趨勢轉壞。'
             '&#10;&#10;bounce_pct (反彈幅度)：從低點到今日的反彈幅度 (e.g., 0.075 = 反彈 7.5%)。越大代表反彈動能越強。'
             '&#10;&#10;min(vol_ratio, 3) (量比)：近 3 日均量 / 急跌期均量；> 1 = 量增。上限 3，避免異常爆量拉高分數。'
             '&#10;&#10;today_vs_peak − 0.7 (距峰值)：今日價格 / 峰值的比率減 0.7 (e.g., 0.9 − 0.7 = 0.2)。越接近 1 (越接近前高)、setup 越完美；< 0.7 表示距峰太遠 setup 失效。'
             '&#10;&#10;範例：rally_gain 0.42 × drop_pct 0.22 × bounce_pct 0.075 × vol 1.28 × 0.2 ≈ 0.0177，屬於中上分數。">第二波分數</th>'
